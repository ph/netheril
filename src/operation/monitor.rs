...
impl Monitor<Queued> {
	...
	self.inner.sender.send(Message::UpdateOperation { id: self.inner.id, state: State::Working}).await?;
	...
	self.inner.sender.send(Message::UpdateOperation { id: self.inner.id, state: State::Canceled }).await?;
...
impl Monitor<Working> {
	...
	self.inner.sender.send(Message::UpdateOperation { id: self.inner.id, state: State::Failed }).await?;
	...
	self.inner.sender.send(Message::UpdateOperation { id: self.inner.id, state: State::Completed}).await?;
...
mod test {
	...
	let handle = tokio::spawn(async move {
	    ...
	    assert_eq!(Message::UpdateOperation{ id, state: State::Working}, m);
	});
	...
	let handle = tokio::spawn(async move {
	    let m = rx.recv().await.unwrap();
	    assert_eq!(Message::UpdateOperation{ id, state: State::Canceled}, m);
	});
	...
	let handle = tokio::spawn(async move {
	    ...
	    let m = rx.recv().await.unwrap();
	    assert_eq!(Message::UpdateOperation{ id, state: State::Failed}, m);
	});
